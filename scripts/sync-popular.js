...
async function syncPopular(limit = 20) {
    console.log('========================================');
    console.log('⭐ Sync Popular Anime from AniList');
    console.log('========================================\n');
    ...
    try {
        ...
        console.log(`📊 Fetching top ${limit} popular anime...`);
        const data = await anilistApi.getPopular(1, limit);
        
        if (!data.media || data.media.length === 0) {
            console.log('❌ No popular anime found');
            await SyncLog.update(syncLogId, {
                status: 'FAILED',
                error_message: 'No popular anime found'
            });
            return;
        }

        console.log(`✅ Found ${data.media.length} popular anime\n`);
